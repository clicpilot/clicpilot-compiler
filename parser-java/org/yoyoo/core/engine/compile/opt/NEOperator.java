...
public class NEOperator extends AbstractCompareOperator {

	protected NEOperator(SimpleNode node, CompilationUnit unit,
	...
	protected IRuntimeValueAtom eval(RuntimeContext ctx)
		...
		if (leftObj instanceof YoyooInteger) {
			...
			if(leftObj instanceof YoyooNull && rightObj instanceof YoyooNull ) {
				return new RuntimeValueAtom(new YoyooBoolean(false), node, unit);
			} else {
				return new RuntimeValueAtom(new YoyooBoolean(leftObj != rightObj),
					node, unit);
			}
		}
